...
void caixeiroAux(Grafo*g, int atual, int numVisitados){
  ElemLista* vizinho = g->A[atual]; //atalho para escrever menos código no else
  if (numVisitados == g->numVertices) { //condição para o melhor cenário possível
    if (arestaExiste(g, atual, 0)) { //verifica se há o ciclo hamiltoniano
      // Calcula o custo completo do ciclo
      int custoCiclo = 0;
      for (int i = 0; i < numVisitados - 1; i++) {
        custoCiclo += pesoAresta(g, cicloAtual[i], cicloAtual[i + 1]);
      }
      custoCiclo += pesoAresta(g, cicloAtual[numVisitados - 1], cicloAtual[0]);
      // Atualiza os valores obtidos no ciclo atual caso eles sejam melhores que os anteriores até achar o melhor melhorValor e melhorCiclo
      if (custoCiclo < melhorValor) {
        melhorValor = custoCiclo;
    ...
    // recursão caso a solução não seja a ideal, visita os vizinhos enquanto ainda existem e passando pelos caminhos possíveis recursivamente até achar a melhor solução
    while(vizinho != NULL){
